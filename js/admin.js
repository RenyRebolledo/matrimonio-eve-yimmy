...
async function loadAdminData() {
  // 1. Fetch latest RSVPs from Cloud
  try {
    const url = `https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${GH_RSVP_PATH}?ref=main&t=${Date.now()}`;
...
function downloadAllPhotosBulk() {
  ...
  photos.forEach((p, idx) => {
    setTimeout(() => {
      ...
      link.click();
      document.body.removeChild(link);
    }, idx * 400); // Stagger downloads slightly
  });
}
